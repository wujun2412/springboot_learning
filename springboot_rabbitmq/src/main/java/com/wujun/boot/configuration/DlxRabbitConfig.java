...
public class DlxRabbitConfig {
     ...
     * @return
     */
    @Bean("orderDeadLetterRedirectExchange")
    public Exchange orderDeadLetterRedirectExchange() {
        return ExchangeBuilder.directExchange(DEAD_LETTER_REDIRECT_EXCHANGE).durable(true).build();
    }

    @Bean("orderRedirectQueue")
    public Queue orderRedirectQueue() {
        return new Queue(DEAD_LETTER_REDIRECT_QUEUE, true, false, false);
    }
